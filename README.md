...
JetBrains requires the first Marketplace upload for a plugin to be done manually. After that initial plugin page exists, the `v*` tag workflow signs the complete native ZIP, uploads the signed ZIP to the GitHub Release, and runs `publishPlugin` for Marketplace publishing.
