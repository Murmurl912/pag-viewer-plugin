...
- The upstream `reference/libpag/viewer` application confirms an official Qt desktop viewer path for macOS and Windows, but that build produces a full PAGViewer app with Qt, updater, exporter, and packaging dependencies. The plugin does not need that app stack; it needs only the smaller libpag C ABI shared runtime. CI therefore probes the root `build_pag` path with `PAG_USE_C=ON` and `PAG_BUILD_SHARED=ON` for Linux and Windows before any full multi-platform plugin ZIP is declared.
- `Build Release` GitHub Actions workflow builds/tests the plugin using a downloaded IntelliJ Platform (`-PuseRemotePlatform=true`), verifies the ZIP structure, uploads the ZIP as a workflow artifact, and attaches it to GitHub Releases for `v*` tags.
- `Build Native Runtimes` GitHub Actions workflow is manual and experimental. It builds Linux x86_64 and Windows x86_64 libpag shared-library artifacts and uploads them for target-machine verification. Those artifacts are not bundled into the release ZIP until platform smoke tests prove JNA loading and PAG decoding on each OS.
