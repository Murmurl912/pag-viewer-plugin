...
- The upstream `reference/libpag/viewer` application confirms an official Qt desktop viewer path for macOS and Windows, but that build produces a full PAGViewer app with Qt, updater, exporter, and packaging dependencies. The plugin does not need that app stack; it needs only the smaller libpag C ABI shared runtime. CI therefore uses the root `build_pag` path with `PAG_USE_C=ON` and `PAG_BUILD_SHARED=ON` for Linux and Windows runtime artifacts, while macOS arm64 uses the checked-in local build.
- `Build Release` GitHub Actions workflow builds/tests the plugin using a downloaded IntelliJ Platform (`-PuseRemotePlatform=true`), stages Linux x86_64, Windows x86_64, and checked-in macOS arm64 native runtimes, verifies the ZIP contains every runtime, uploads the ZIP as a workflow artifact, and attaches it to GitHub Releases for `v*` tags. Heavy native runtime builds are cached as GitHub Release assets under a `native-<runtime-key>` prerelease; the key is derived from the libpag submodule commit and native build scripts, so native runtimes rebuild only when those inputs change.
